...
SPANE is a powerful workflow orchestration engine built on [BullMQ](https://bullmq.io/) and [Redis](https://redis.io/). It executes DAG-based (Directed Acyclic Graph) workflows with parallel node processing, automatic retries, state persistence, and comprehensive error handling.
